...
class TimelineController extends Controller
{
    ...
    public function editInternal($id_timeline)
    {
        ...
        $eis = EventInternal::all();
        $type = "eventinternal";
        $title = "Update Timeline Event Internal";
        if ($tl) {
            if (Session::get('is_pembina') == "0") {
                $navTitle = '<i class="icon-copy dw dw-time-management mr-2"></i>Update Timeline Event';
            } else {
                $navTitle = '<i class="icon-copy dw dw-time-management mr-2"></i>Detail Timeline Event';
            }
    ...
    public function editEksternal($id_timeline)
    {
        ...
        $ees = EventEksternal::all();
        $type = "eventeksternal";
        $title = "Update Timeline Event Eksternal";
        if ($tl) {
            if (Session::get('is_pembina') == "0") {
                $navTitle = '<i class="icon-copy dw dw-time-management mr-2"></i>Update Timeline Event';
            } else {
            ...
            return view('ormawa.timeline.edit', compact('ees', 'navTitle', 'tl', 'type', 'title'));
        } else {
