                        ...
                        <div class="pd-20">
                            ...
                            <div class="row mt-5" id="container-event-active">
                                @foreach ($favs as $fav)
                                    <div class="col-sm-12 col-md-6 col-lg-4 mb-30">
                                        <div class="komp-card2 mx-auto">
                                            <div class="komp-thumbnail" data-background="{{asset('assets/img/kompetisi-thumb/'.$fav->eventEksternalRef->poster_image)}}">
                                            </div>
                                            <div class="komp-banner">
                                                <div class="komp-banner__date text-left pl-2 pt-1" >  
                                                    @php
                                                        $tgl_buka = Carbon\Carbon::parse($fav->eventEksternalRef->tgl_buka)->toDatetime()->format('M, d Y');
                                                    @endphp
                                                    {{$tgl_buka}}
                                                </div>
                                                <div class="komp-banner__date text-right pr-2 pt-1" data-toggle="tooltip" data-placement="top" title="{{$fav->eventEksternalRef->cakupanOrmawaRef->ormawaRef->nama_ormawa}}">
                                                    @php
                                                        $count = str_word_count($fav->eventEksternalRef->cakupanOrmawaRef->ormawaRef->nama_ormawa);
                                                        $nama_ormawa = $fav->eventEksternalRef->cakupanOrmawaRef->ormawaRef->nama_ormawa;
                                                        if($count > 3){
                                                            $nama_ormawa = $fav->eventEksternalRef->cakupanOrmawaRef->ormawaRef->nama_akronim;
                                                        }
                                                    @endphp
                                                    {{$nama_ormawa}}
                                                </div>
                                            ...
                                            <div class="komp-title pl-3">
                                                @php
                                                     $slug = \Str::slug($fav->eventEksternalRef->nama_event);
                                                @endphp
                                                <a href="{{route('peserta.eventeksternal.detail', $slug)}}"><h1 class="mt-3">
                                                    {{$fav->eventEksternalRef->nama_event}} 
                                                </h1></a>
                                            </div>
                                            <div class="komp-description pl-3 pr-3 mt-2">
                                                {!!$fav->eventEksternalRef->deskripsi_excerpt!!}
                                            </div>
                                            <div class="komp-created pl-3 pr-3 mt-3">
                                                <p class="text-secondary d-inline">
